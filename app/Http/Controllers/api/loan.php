...
class loan extends Controller
{
	...
	public function doApiregisterApplyloan()
	{
		...
		if(Input::get('page_name')=='applyloan')
		{
			...
			if(!empty($schemaname))
			{
				...
				$fetchupdate 				= 	loanmodel::doGetmasterdetails(
															$this->dmsdealerTable,
    ...
    public function getschemaname($id)
    {
     ...
     public static function loanapitest($schemaId="",$cusotmerId="",$uniqueId="",$loanAmount="",$carId)
 	{
 		// $customerId 	= $request->cusotmerId;
 		// $loanAmount 	= $request->loanAmount;
 		$schemaname 	=  $schemaId;
 		$customerId 	= $cusotmerId;
 		$loanAmount 	= $loanAmount;
 		$cardDetails 	= loanmodel::getModelID($schemaname,$carId);

 		
 			$isNewVehicle 	= "true";
	 		$remarks 		= "Apply for Loan";
	 		$sourceID 		= "DEALERPLUSPORTAL";
	 		$soruceRefID 	= "12595";
			//$uniqueID 		= "";
			$uniqueID 		= $uniqueId;
			$categoryID 	= 4;

	 		$loanApi 		= loanmodel::loanapitest($schemaname,$customerId);
	 		//$modelName 		= loanmodel::getModelName($schemaname,$cardDetails[0]->model_id);
	 		$errorCode		= "";
	 		$message 		= "";
	 		$data 			= array();
	 		foreach ($loanApi as $key => $value) {
	 			$data['firstName'] = $value->contact_first_name;
	 			$data['lastName'] = $value->contact_last_name;
	 			$data['mobileNumber'] = $value->contact_phone_1;
	 			$data['landLineNo'] = $value->contact_phone_2;
	 			$data['dateOfBirth'] = '';
	 			$data['emailID'] = $value->contact_email_1;
	 			$data['gender'] = $value->contact_gender;
	 			$data['employment'] = $value->employee_information_type;
	 			$data['addressDetails'] = array('addressLine1'=>$value->contact_mailing_address,
	 											'addressLine2'=>$value->contact_mailing_locality,
	 											'addressLine3'=>'',
	 											'city'=>$value->contact_mailing_city,
	 											'pincode'=>$value->contact_mailing_pincode,
	 											'state'=>'');
	 			$data['loanAmount'] = $loanAmount;
	 			$data['vehicle'] = array('isNewVehicle'=>$isNewVehicle,
	 									 'modelID'=>$cardDetails[0]->model_id,
	 									 'categoryID'=>$categoryID,
	 									 'registrationNo'=>$cardDetails[0]->registration_number);
	 			$data['remarks'] = $remarks;
	 			$data['sourceID'] = $sourceID;
	 			$data['soruceRefID'] = $soruceRefID;
			...
			if($errorCode == "")
			{
				...
				//$leadreponse = (new \App\http\Controllers\api\loan)->leadapi($jsondata);
				$updatedata	  =	array('lead_id'=>12);
				$updateId = array('dealer_customer_loan_id'=>$uniqueID);
				$updateDetails = loanmodel::doRevokeloantable($updatedata,$updateId);
				return 1;
				/*if(count($leadreponse)>0)
				{
					//return $leadreponse->leadID;
					return 1;
				}
				else{
					return 0;
 	...
 	public function leadapi($jsondata)
 	{
 		return "3";
 	// 	$ch=curl_init();
 	// 	curl_setopt($ch,CURLOPT_URL,'http://10.9.56.44:8011/DealerPortal/NewLead?userID=DealerPortal&&selectedRoleID=SYSTEM');
 	...
 	public function leadApiTestAjax(Request $request)
 	{
 		$leadId = $request->leadId;
 		$leaddata = $this->leadApiStatus($leadId);
 		$leaddata = json_decode($leaddata);
 		return view('leadApiTestAjax',compact('leaddata'));
 		//return $this->leadApiStatus($leadId);
 	}

 	public function leadApiTest($leadId="")
 	{

 		/*$area = json_decode($jsondata);
 		*/
 		//return $area;
		//print_r($jsondata);
		//print_r($area->workflow);
 		//dd(Input::get('leadId');

 		$leadStatus = (new \App\http\Controllers\api\loan)->leadApiStatus($leadId);
 		$area = json_decode($leadStatus);
 		//return $leadStatus;
 		$datastatus;
 		foreach ($area->workflow as $key => $value) {
 			$datastatus = $actionTaken = $value->actionTaken;
 			//print_r($value->actionTaken);
 		}
 	...
 	public function leadApiStatus($leadId)
 	{
 	// 	$ch=curl_init();
 	// 	curl_setopt($ch,CURLOPT_URL,'http://10.9.50.57:3005/leap/salesapi/v1.0/vf/leads/tracking?leadID=leadId');
		// curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1); 
		// curl_setopt($ch, CURLOPT_CONNECTTIMEOUT , 7);
		// curl_setopt($ch, CURLOPT_HEADER, 0);
		// $content = curl_exec($ch); 
		...
		$leadApi 		= loanmodel::leadApiTestStatus($leadId,$schemaname);
 		//dd($leadApi);
 		$loansatus      = array();
 		$data 			= array();
 		$workflow       = array();
 		$leadid = md5($leadApi[0]->contact_management_id);

		foreach ($leadApi as $key => $value) {
 			$workflowa = array(
	 			'role' => $value->contact_type,
	 			'userID' => $value->contact_management_id,
	 			"name"=> $value->contact_first_name,
				"startDate"=> "03-07-2017 12:43 PM",
				"endDate"=> "03-07-2017 12:43 PM",
				"actionTaken"=> "preSanctionPending".$key,
				"sentTo"=> array(
				    "role"=> $value->contact_type_id,
				    "userID"=> $value->contact_management_id,
				    "userName"=> $value->contact_owner
				),
				"remarks"=> "",
			);
			array_push($workflow,$workflowa);

		}
		$loansatus['workflow'] = $workflow;
		$loansatus['leadid'] = $leadid;
		$loansatus["agreementNos"]= array(
				"lmsResponse"=>"XTRAVYA00001922377",
				"loanID"=>md5($leadApi[0]->dealer_customer_loan_id)
			);
 		$jsondata = json_encode($loansatus);
 		return $jsondata;
 	...
 	public function createLead()
 	{
 		...
 		$carid 		= Input::get('carId');
 		$loanAmt 	= Input::get('loanAmt');
 		//echo($loanAmt);
 		(new LoanApiController)->loanapitest($schemaname,$customerId,$loanId,$loanAmt,$carid);
 		//echo ($carid);
 	}
}
...
class loan extends Controller
{
    ...
    public function doViewloanPage()
    {
    	try
		{
			...
			$left_menu    		= 	'2';
			$leadstatus 		= (new LoanApiController)->leadApiTest($getloanresult[0]->lead_id);
			...
			return view('applied_loan_view', compact('compact_array', 'header_data','leadstatus'));
    }
		...
		if($insertticketrequesttable >= 1)
		{
			...
			if($insertdealerloantable >= 1)
			{
				...
				if($updatedmstable >= 1)
				{
					...
					if($loanApiQueue == "YES")
					{
						$apiResult = new \App\Jobs\loanApiQueue($this->dealer_schemaname,$customerid,$amount,DB::getPdo()->lastInsertId(),$carid);
						dispatch($apiResult);
					}
    ...
    public function doexport_excel_loan()
    {
    	try
		{
			...
			$mergecells 		= "A1:M1";
			$sheetArray 		= array();
			$sheetArray[] 		= array('SL No','Customer Name','Customer Mobile Number','Customer Email','Customer PAN','Vehicle Details','Amount','Date','City','Branch Name','Loan Ticket ID','Ticket Status','Lead Id');
			foreach($fetch_data  as $i => $row)
			{
				...
				$sheetArray[] 	= array(++$i,$row->customername,$row->customermobileno,$row->customermailid,$panNumber,$row->vehicle_details,$amount,$row->created_date,$row->customercity,$row->branchname,$row->dealer_loan_ticket_id,$row->ticketstatus,$row->lead_id);
			}

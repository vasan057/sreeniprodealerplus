                            ...
                            <tbody>
                                ...
                                @foreach($compact_array['loandata'] as $fetch)
                                    <tr>
                                        ...
                                        <td data-title="Lead Id">{{$fetch->lead_id}}</td>
    									<td data-title="Ticket Status" class="revokeditmes">
                                        @if(isset($fetch->lead_id))
                                            <a href="#myModal" data-toggle="modal" id="{{$fetch->lead_id}}" data-target="#popup" class="btn btn-danger">More Info</a>
                                        @else
                                            <a data-id="{{$fetch->customer_id}}" data-loanId="{{$fetch->dealer_customer_loan_id}}" data-loanAmount="{{$fetch->requested_amount}}" data-carid="{{$fetch->car_id}}"  class="btn btn-primary createLead">Create Lead</a>
                                        @endif
                                        </td>
...
<script type="text/javascript">
    ...
    $("body").on("click", ".createLead", function () {
        $("#loadspinner").css("display", "block");
        var customerId =($(this).attr('data-id')) ;
        var loanid =($(this).attr('data-loanId')) ;
        var loanAmt =($(this).attr('data-loanAmount')) ;
        ...
        $.ajax({
            ...
            url:"{{url('/createLead')}}",
            datatype:'JSON',
            data: {customerId: customerId,carId:carId,loanAmt:loanAmt,loanid:loanid},
            
            success:function(data)
            {
                $("#loadspinner").css("display", "none").hide();
                if(data == 0)
                {
                    $(".successmsg").css("display", "none");
                    $(".errormessage").css("display", "block").show();
                    $(".errormessage").html("Fail to create Lead ").show(); 
                    setTimeout(function(){ 
                        $(".successmsg").css("display", "none").hide();
                        $(".errormessage").css("display", "block").hide();
                        $(".errormessage").html("Fail to create Lead ").hide(); 
                    }, 5000);
                }
                else
                {
                    $(thisid).parent().html('Pending').show();
                    $(".errormessage").css("display", "none");
                    $(".successmsg").css("display", "block").show();
                    $(".successmsg").html("Successfully Created Lead").show();
                    setTimeout(function(){ 
                        $(thisid).parent().html('Pending').hide();
                        $(".errormessage").css("display", "none").hide();
                        $(".successmsg").css("display", "block").hide();
                        $(".successmsg").html("Successfully Created Lead").hide();
                    }, 5000);
                }
            },
            error: function () 
    ...
    $('#popup').on('show.bs.modal', function (e)
    {
        ...
        $.ajax({
            type: 'POST',
            headers: {'X-CSRF-TOKEN': token},
            url: "{{url('/leadApiTestAjax')}}?leadId="+esseyId,
            data: {id: esseyId},
            datatype: 'JSON',
            success: function (data)
            {
                $('#leadstatus').empty().append(data); 
            }
            // error: function () 
            // {
            //     alert();
